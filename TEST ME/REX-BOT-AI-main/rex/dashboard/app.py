...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # NOTE: InterviewService is created and managed by the orchestrator.
    # The dashboard should NOT create a second instance -- doing so causes
    # two consumers to race on the same Redis streams.  The interview router
    # accesses the service via deps.get_interview_service(), which the
    # orchestrator (or an external caller) is responsible for populating.
