...
def verify_password(password: str, hashed: str) -> bool:
    ...
    """
    _reject_null_bytes(password)
    # Try current scheme (SHA-256 pre-hashed) first, then fall back to legacy
    # (raw password fed directly to bcrypt).
...
def _is_legacy_hash(password: str, pw_hash: str) -> bool:
    """Detect whether *pw_hash* was created without SHA-256 pre-hashing.

    Current scheme: ``bcrypt(base64(sha256(password)))``.
    Legacy  scheme: ``bcrypt(password)``  (raw password fed to bcrypt).

    If the hash does NOT verify under the current pre-hashed scheme but DOES
    verify with the raw password, it is a legacy hash that should be upgraded.
    """
    hashed_bytes = pw_hash.encode()
    # Matches current (pre-hashed) scheme -- not legacy.
    if bcrypt.checkpw(_prehash_password(password), hashed_bytes):
        return False
    # Matches raw-password scheme -- this is a legacy hash.
