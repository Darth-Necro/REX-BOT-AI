...
class TestAutoRestart:
    ...
    async def test_auto_restart_second_attempt(self) -> None:
        ...
        svc = _mock_service(ServiceName.STORE)
        orch.register(svc)
        # Simulate one recent restart in the sliding window
        orch._restart_timestamps[ServiceName.STORE] = [_time.monotonic() - 1]
    ...
    async def test_auto_restart_disables_at_max(self) -> None:
        """After MAX_RESTART_ATTEMPTS within the window, service should be disabled."""
        import time as _time
        ...
        orch.register(svc)
        now = _time.monotonic()
        orch._restart_timestamps[ServiceName.STORE] = [now - 10, now - 5, now - 1]
...
class TestRunMethod:
    ...
    async def test_run_starts_services_and_blocks(self) -> None:
        ...
        with (
            ...
            patch("rex.core.orchestrator.asyncio.create_task", side_effect=_closing_create_task),
    ...
    async def test_run_creates_health_monitor_task(self) -> None:
        ...
        with (
            patch("rex.core.orchestrator.asyncio.Event", return_value=mock_event),
            patch("rex.core.orchestrator.asyncio.create_task", side_effect=_tracking_create_task),
            patch.object(orch, "stop_all", new_callable=AsyncMock),
        ):
            await orch.run()
